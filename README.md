...
- **업데이트 날짜 + 오픈이슈**: 각 카드 본문 맨 아래에 좌측 `업데이트: <날짜>` (클라이언트 로드 시 상대 시간으로 변환 — `방금 전`, `N시간 전`, `1일 전`, `N일 전`, 30일 이상은 월/일 표기. 언어 전환 시 함께 갱신), **같은 라인 우측 끝**에 `오픈이슈: <N>` 표시 (GitHub `open_issues_count` 원본 — 오픈된 Issue + Pull Request 합산, 0 포함 항상 노출)
- **북마크**: 카드 좌상단 별(★) 버튼으로 즐겨찾기 토글 — 북마크된 저장소는 정렬 결과의 **최상단으로 고정**. 상태는 브라우저 `localStorage` 에 보관 (서버 저장 X)
- **상단 통계 바**: 히어로 바로 아래에 4개 지표 표시 — `활성 프로젝트`(전체 저장소 수), `열린 이슈`(전체 저장소 오픈이슈 합계), `시스템 가동률`(`99.7%` 고정 표기), `최근 업데이트`(카드 `업데이트`와 동일 기준 — 전체 저장소 `updated_at` 중 가장 최근 값의 상대 시간)
...
│   │   ├── StatsBar.astro          # 메인 상단 통계 바 (활성 프로젝트/열린 이슈/시스템 가동률/최근 업데이트)
...
| **카드/상세 표시명 교체** | 본 저장소의 `src/data/repo-overrides.json` 에 `displayName` 추가 | 카드 제목 + 상세 페이지 헤더 + 페이지 title + OG 메타 노출명 교체 (URL 경로/검색 인덱스는 영향 없음) |
...
- **카드/상세 표시명 교체 규칙 (overrides.displayName)** — "Display Name 관리" 섹션 참고
...
GitHub 저장소 이름은 URL/식별자로 사용되어 공백·대소문자 변경이 자유롭지 않지만, **사용자에게 노출되는 표시명**은 `displayName` 오버라이드로 자유롭게 교체할 수 있습니다.

### 적용 위치 (사용자 노출 텍스트만)
- 카드 제목 (메인 페이지)
- 카드 이미지 `alt` (스크린리더)
- 상세 페이지 상단 시안색 저장소 이름
- 브라우저 페이지 `<title>` (`<displayName> — BIT LEADER`)
- 검색 엔진용 JSON-LD `name`
- OG 이미지 안의 큰 글자 (SNS/메신저 공유 시 미리보기 카드)

### 영향받지 않는 곳 (식별자/검색 인덱스는 원본 유지)
- 상세 페이지 URL 경로 (`/bitleader/<repo-name>/`)
- 카드 검색 인덱스 `data-name`
- 북마크 키 (localStorage)
- OG 이미지 **파일명**(`/og/<repo-name>.png`) — 안의 큰 글자만 displayName 으로 교체됨
- GitHub API 호출 키 (`fetchReadme(<repo-name>)` 등)
...
{
  "DokkaebiTerminal": {
    "displayName": "Dokkaebi Terminal"
  }
}
```

- **key**: GitHub 저장소 이름 (대소문자 그대로)
- **displayName**: 카드/상세 페이지에 노출할 표시명 (공백·대소문자 자유)
- 동일 저장소 키에 `downloadUrl` 과 `displayName` 을 함께 작성 가능

### 동작 예시

| 저장소 | overrides.displayName | 카드/상세 헤더 노출 | URL 경로 |
|---|---|---|---|
| `DokkaebiTerminal` | `"Dokkaebi Terminal"` | **Dokkaebi Terminal** | `/bitleader/DokkaebiTerminal/` |
| `ClipMemory-releases` | 없음 | `ClipMemory-releases` | `/bitleader/ClipMemory-releases/` |

> 검색창에서 `dokkaebi terminal`(공백 포함) 으로는 매칭되지 않습니다 — 검색 인덱스는 원본 저장소 이름 기준입니다.
