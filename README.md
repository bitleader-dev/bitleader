...
- **상단 통계 바**: 히어로 바로 아래에 4개 지표 표시 — `활성 프로젝트`(전체 저장소 수), `열린 이슈`(전체 저장소 오픈이슈 합계), `시스템 가동률`(`99.7%` 고정 표기), `최근 배포`(전체 저장소 중 가장 최근 발행된 릴리스의 상대 시간)
...
│   │   ├── StatsBar.astro          # 메인 상단 통계 바 (활성 프로젝트/열린 이슈/시스템 가동률/최근 배포)
